		...
		case UnaryExpr:
			...
			case mtoken.UNQUOTE, mtoken.UNQUOTE_SPLICE:
				...
				env.debugQuasiQuote(fmt.Sprintf("inside %s, lastUnquote is %s (unquoteDepth = %d)",
					mtoken.String(child.Op()), mtoken.String(op), unquoteDepth), depth, canSplice, child)

				if unquoteDepth > depth {
					env.Errorf("%s not inside quasiquote: %v <%v>", mtoken.String(op), lastUnquote, r.TypeOf(lastUnquote))
					return nil
				} else if unquoteDepth < depth {
					toexpand := child.Get(0).Get(1)
					env.debugQuasiQuote(fmt.Sprintf("recursing inside %s, lastUnquote is %s", mtoken.String(child.Op()), mtoken.String(op)),
					...
					toInsert := AnyToAst(env.evalUnquote(lastUnquote), mtoken.String(op))
					if toInsert == nil {
