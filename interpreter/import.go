...
func (ir *Interpreter) writeImportFile(out io.Writer, path string, pkg *types.Package, internal bool) (isEmpty bool) {
	scope := pkg.Scope()
	names := scope.Names()
...
func init() {
	Binds[%q] = map[string]Value{`, path)
		...
		fmt.Fprint(out, `
func Package() (map[string]Value, map[string]Type) {
	...
	for _, name := range names {
		if obj := scope.Lookup(name); obj.Exported() {
			...
			case *types.Const:
				...
				fmt.Fprintf(out, "\n\t\t%q:\tValueOf(%spkg.%s%s),", name, prefix, name, suffix)
			case *types.Var:
				fmt.Fprintf(out, "\n\t\t%q:\tValueOf(&pkg.%s).Elem(),", name, name)
			case *types.Func:
	...
	for _, name := range names {
		if obj := scope.Lookup(name); obj.Exported() {
			switch obj.(type) {
			case *types.TypeName:
				fmt.Fprintf(out, "\n\t\t%q:\tTypeOf((*pkg.%s)(nil)).Elem(),", name, name)
			}
		}
	}
	fmt.Fprint(out, "\n\t}\n}\n")
