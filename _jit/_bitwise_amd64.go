...
// %reg_z &= a
func (asm *Asm) And(dst Arg, a Arg, b Arg) *Asm {
	lo, hi := asm.lohi(z)
	if a.Const() {
		val := a.(*Const).val
		if val == 0 {
...
// %reg_z |= a
func (asm *Asm) Or(dst Arg, a Arg, b Arg) *Asm {
	lo, hi := asm.lohi(z)
	if a.Const() {
		val := a.(*Const).val
		if val == 0 {
...
// %reg_z ^= a
func (asm *Asm) Xor(dst Arg, a Arg, b Arg) *Asm {
	lo, hi := asm.lohi(z)
	if a.Const() {
		val := a.(*Const).val
		if val == 0 {
...
// %reg_z &^= a
func (asm *Asm) Andnot(dst Arg, a Arg, b Arg) *Asm {
	lo, hi := asm.lohi(z)
	var tmp hwReg
	if a.Const() {
...
// dst = ^ src
func (asm *Asm) Not(dst Arg, src Arg) *Asm {
	return asm
}
